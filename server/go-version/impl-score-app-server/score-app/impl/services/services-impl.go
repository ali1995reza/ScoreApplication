...
func (j *JwtAuthenticationService) CreateToken(userId string, expireAfter int64) (string, bool) {
	...
	tokenString, ex := token.SignedString(j.secret)
	if ex != nil {
		return "", false
	}
...
func (j JwtAuthenticationService) ValidateToken(tokenString string) (string, exceptions.ScoreApplicationException) {
	...
	if err != nil || !token.Valid {
		return "", exceptions.NewAuthenticationTokenInvalidException("invalid token")
	}
	...
	if time.Now().UnixMilli() > int64(expirationTime) {
		return "", exceptions.NewAuthenticationTokenExpiredException("token expired")
	}
	return userId, nil
}
